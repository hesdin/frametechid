...
class UpdatePricingPlanRequest extends FormRequest
{
    ...
    {
        /** @var PricingPlan|null $pricingPlan */
        $pricingPlan = $this->route('pricing_plan');

        return [
            'name' => ['required', 'string', 'max:255'],
            'slug' => ['required', 'string', 'max:255', 'regex:/^[a-z0-9-]+$/', 'unique:pricing_plans,slug,'.($pricingPlan?->id ?? 'NULL')],
