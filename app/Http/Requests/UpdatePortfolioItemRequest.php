...
class UpdatePortfolioItemRequest extends FormRequest
{
    ...
    {
        /** @var PortfolioItem|null $portfolioItem */
        $portfolioItem = $this->route('portfolio_item');

        return [
            'title' => ['required', 'string', 'max:255'],
            'slug' => ['required', 'string', 'max:255', 'regex:/^[a-z0-9-]+$/', 'unique:portfolio_items,slug,'.($portfolioItem?->id ?? 'NULL')],
